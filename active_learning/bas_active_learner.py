...
class BASActiveLearner(BaseActiveLearner):
    ...
    def select_idxs(self) -> List[int]:
        ...
        sample_dataset, reduced_sample_dataset_idxs = self.data_handler.sample_from_unlabeled(bas_num_samples_to_rank)
        ...
        summary_column = self.dataset_cfg.target_col
        al_dataset = self.data_handler.get_dataset()
    ...
    def compute_bleuvar_scores(self, sample_dataset_idxs: List[int]) -> List[float]:
        sample_dataset = self.data_handler.get_dataset().select(sample_dataset_idxs)
        assert(sample_dataset[0][self.dataset_cfg.target_col] == self.data_handler.dataset[sample_dataset_idxs[0]][self.dataset_cfg.target_col])
    ...
    def _run_mc_dropout(self, batch: dict) -> List[List[str]]:
        ...
        for i_s in range(self.active_learning_cfg.bas_num_samples_mc_dropout):
    ...
    def _analyze_generation_bleuvar(self, gen_list: List[str]) -> float:
        ...
        """
        n = self.active_learning_cfg.bas_num_samples_mc_dropout
        bleu_var = 0.
